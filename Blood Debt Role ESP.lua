    ...
    local function hasWeapon(tbl)
        if bp then
            for _, tool in ipairs(bp:GetChildren()) do
                if tool:IsA("Tool") and tbl[tool.Name] then return true end
            end
        end
        if char then
...
-- Update humanoid every time your character respawns
LocalPlayer.CharacterAdded:Connect(function(char)
    Humanoid = char:WaitForChild("Humanoid")
end)

-- Make sure humanoid is always initialized
if LocalPlayer.Character then
    Humanoid = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
end

local infJumpSection = MovementTab:CreateSection("Infinite Jump")
...
MovementTab:CreateToggle({
    ...
    Callback = function(state)
        ...
        if state and Humanoid then
            task.spawn(function()
                -- Sometimes setting immediately errors, so wrap safely
                pcall(function()
                    Humanoid:SetStateEnabled(Enum.HumanoidStateType.Jumping, true)
                    Humanoid:SetStateEnabled(Enum.HumanoidStateType.Landing, true)
                end)
            end)
        end
    end,
